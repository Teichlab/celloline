...
if __name__ == "__main__":
    ...
    group1.add_argument('-m','--mapping', help='Mapping algorithm, currently available are gmap, bowtie, bowtie2, tophat, tophat2 and STAR', required = True)
    group1.add_argument('-margs','--mapping_args', help='Mapping algorithm additional arguments', nargs="+")
    group1.add_argument('-q','--quantification', help='Quantification algorithm. Currently accepted are HTSeq and cufflinks')
    ...
    group1.add_argument('--overwrite', dest="overwrite",help='Overwrite files during run. Will not work for creating index', action="store_true")
    ...
    group2.add_argument('-f','--fasta', help='Full path to fasta files. Providing this  argument will make the pipline produce index files for the specified mapper and stop. Must be given with -gtf')
    group2.add_argument('-gtf', help='Full path to gene annotation file. Prividing this will make the pipline produce index files for the specified mapper and stop. Must be given with -f')
    
    p.add_argument('-c', '--config', help='Config file', required = True)
    p.add_argument('-ram','--ram', help= 'The amout of ram (MB) that each job (single cell) should allocate (defalut EBI 10000, defalut  AWS: 900) On AWS this is interpreted quite opposite, as the amout of RAM that has to be free before starting a new job.')
    p.add_argument('-cpu','--cpu', help='Number of cpus that each job (single cell) in the queue is allowed to use. On AWS this number should optimaly be multiple of the number of cores on the instance (default 2). Providing >> 8 cores might decrease overall efficiancy as the bottleneck will be i/o ', required = True)
